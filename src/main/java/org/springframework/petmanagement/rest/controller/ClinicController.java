...
public class ClinicController implements ClinicsApi {
    ...
    @Override
    public ResponseEntity<ClinicDto> addClinic(@Valid ClinicFieldsDto clinicFieldsDto) {
    ...
    @PreAuthorize("hasRole(@roles.ADMIN)")
    @Override
    public ResponseEntity<ClinicDto> updateClinic(UUID clinicId, @Valid ClinicFieldsDto clinicFieldsDto) {
        Optional<Clinic> clinicOpt = clinicService.findById(clinicId);
